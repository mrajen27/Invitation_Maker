...
class NamingSampleExportTest {

    @Test
    fun exportNamingWithPhotoAndFullText() {
        ...
        val details = InvitationDetails(
            ...
            message = "Your blessings mean the world to our family."
        )
    ...
    private fun resolvePreviewOutputDir(): File {
        ...
        val moduleDir = File(System.getProperty("user.dir"))
        val repoRoot = if (moduleDir.name == "app") moduleDir.parentFile!! else moduleDir
        return File(repoRoot, "docs/samples/naming")
    }

    private fun copySamplePhoto(cacheDir: File): Uri {
        val outFile = File(cacheDir, "naming_sample_portrait.png")
        if (!outFile.exists()) {
