...
object InvitationBackgroundPainter {
    fun draw(canvas: Canvas, templateId: String, width: Int, height: Int) {
        when (templateId) {
            ...
            in engagementIds -> drawEngagementTheme(canvas, width, height, templateId)
            in namingIds -> drawNamingTheme(canvas, width, height, templateId)
            in babyShowerIds -> drawBabyShowerTheme(canvas, width, height, templateId)
    ...
    private val engagementIds = setOf(
        "engagement_01", "engagement_02", "engagement_03", "engagement_04", "engagement_05"
    )
    private val namingIds = setOf(
        "naming_01", "naming_02", "naming_03", "naming_04", "naming_05"
    )
    private val babyShowerIds = setOf(
        "babyshower_01", "babyshower_02", "babyshower_03", "babyshower_04", "babyshower_05"
    )
    ...
    private fun drawEngagementTheme(canvas: Canvas, width: Int, height: Int, templateId: String) {
        when (templateId) {
            "engagement_01" -> {
                fillGradient(canvas, width, height, Color.parseColor("#FFF5F7"), Color.parseColor("#FFFFFF"))
                drawWatercolorRose(canvas, width - 130f, 170f, 1.05f)
                drawWatercolorRose(canvas, 120f, height - 200f, 0.9f)
                drawLeafSprig(canvas, 70f, 200f)
                drawGoldOvalFrame(canvas, width / 2f, 210f, 150f, 95f)
            }
            "engagement_02" -> {
                fillGradient(canvas, width, height, Color.parseColor("#F1F8E9"), Color.parseColor("#FDFEF8"))
                drawLeafSprig(canvas, 55f, 160f)
                drawLeafSprig(canvas, width - 55f, height - 210f, flip = true)
                drawWatercolorRose(canvas, width - 110f, 180f, 0.75f)
                drawGoldOvalFrame(canvas, width / 2f, 205f, 165f, 100f)
            }
            "engagement_03" -> {
                fillGradient(canvas, width, height, Color.parseColor("#0D3B47"), Color.parseColor("#1A5C66"))
                drawBokeh(canvas, width, height, seed = 71)
                drawLanternString(canvas, width, 130f, warm = true)
                drawWatercolorRose(canvas, 100f, height - 190f, 0.8f)
                drawGoldHexFrame(canvas, width, height, inset = 64f, alpha = 180)
            }
            "engagement_04" -> {
                fillGradient(canvas, width, height, Color.parseColor("#F3E5F5"), Color.parseColor("#FDF8FF"))
                drawGarland(canvas, width, 105f)
                drawWatercolorRose(canvas, 95f, 175f, 0.85f)
                drawWatercolorRose(canvas, width - 95f, 175f, 0.85f)
                drawGoldHexFrame(canvas, width, height, inset = 70f, alpha = 130)
            }
            else -> {
                fillGradient(canvas, width, height, Color.parseColor("#F9FBE7"), Color.parseColor("#FFFFFF"))
                drawMangoLeaves(canvas, width, 95f)
                drawLanternString(canvas, width, 125f, warm = true)
                drawLeafSprig(canvas, 60f, height - 220f)
                drawLeafSprig(canvas, width - 60f, height - 220f, flip = true)
                drawGoldHexFrame(canvas, width, height, inset = 72f, alpha = 140)
            }
        }
    }

    private fun drawNamingTheme(canvas: Canvas, width: Int, height: Int, templateId: String) {
        when (templateId) {
            "naming_01" -> {
                fillGradient(canvas, width, height, Color.parseColor("#FFF8E1"), Color.parseColor("#FFFBF0"))
                drawGarland(canvas, width, 108f)
                drawKolamRing(canvas, width / 2f, 200f, 52f)
                drawLampRow(canvas, width, height - 185f)
                drawGoldHexFrame(canvas, width, height, inset = 74f, alpha = 120)
            }
            "naming_02" -> {
                fillGradient(canvas, width, height, Color.parseColor("#E3F2FD"), Color.parseColor("#F5FAFF"))
                drawSoftClouds(canvas, width, 120f)
                drawBalloonCluster(canvas, width - 100f, 220f, 48f, Color.parseColor("#64B5F6"), Color.parseColor("#1E88E5"))
                drawBalloonCluster(canvas, 95f, 240f, 42f, Color.parseColor("#90CAF9"), Color.parseColor("#42A5F5"))
                drawGoldHexFrame(canvas, width, height, inset = 76f, alpha = 110)
            }
            "naming_03" -> {
                fillGradient(canvas, width, height, Color.parseColor("#FFF9C4"), Color.parseColor("#FFFDE7"))
                drawMangoLeaves(canvas, width, 100f)
                drawLampRow(canvas, width, 150f)
                drawTempleFrame(canvas, width, height)
                drawGoldHexFrame(canvas, width, height, inset = 68f, alpha = 130)
            }
            "naming_04" -> {
                fillGradient(canvas, width, height, Color.parseColor("#FCE4EC"), Color.parseColor("#FFF5F8"))
                drawSoftClouds(canvas, width, 115f)
                drawStarAccent(canvas, width / 2f, 95f)
                drawGarland(canvas, width, height - 120f)
                drawGoldOvalFrame(canvas, width / 2f, 215f, 140f, 88f)
            }
            else -> {
                fillGradient(canvas, width, height, Color.parseColor("#E8F5E9"), Color.parseColor("#F6FBF4"))
                drawWreathRing(canvas, width / 2f, 205f, 120f)
                drawGoldHexFrame(canvas, width, height, inset = 78f, alpha = 100)
            }
        }
    }

    private fun drawBabyShowerTheme(canvas: Canvas, width: Int, height: Int, templateId: String) {
        when (templateId) {
            "babyshower_01" -> {
                fillGradient(canvas, width, height, Color.parseColor("#FCE4EC"), Color.parseColor("#FFF8FB"))
                drawBalloonCluster(canvas, 90f, 210f, 55f, Color.parseColor("#F48FB1"), Color.parseColor("#EC407A"))
                drawBalloonCluster(canvas, width - 95f, 230f, 52f, Color.parseColor("#F8BBD0"), Color.parseColor("#E91E63"))
                drawConfetti(canvas, width, height, seed = 81, alpha = 70)
                drawGoldHexFrame(canvas, width, height, inset = 72f, alpha = 120)
            }
            "babyshower_02" -> {
                fillGradient(canvas, width, height, Color.parseColor("#E3F2FD"), Color.parseColor("#F5FAFF"))
                drawBalloonCluster(canvas, 100f, 220f, 58f, Color.parseColor("#64B5F6"), Color.parseColor("#1565C0"))
                drawBalloonCluster(canvas, width - 105f, 240f, 54f, Color.parseColor("#90CAF9"), Color.parseColor("#1E88E5"))
                drawSoftClouds(canvas, width, 130f)
                drawGoldHexFrame(canvas, width, height, inset = 74f, alpha = 115)
            }
            "babyshower_03" -> {
                fillGradient(canvas, width, height, Color.parseColor("#FFF8E7"), Color.parseColor("#FFFFFF"))
                drawGiftBox(canvas, width - 120f, height - 140f, Color.parseColor("#FFCC80"))
                drawGiftBox(canvas, 100f, height - 150f, Color.parseColor("#90CAF9"))
                drawConfetti(canvas, width, height, seed = 92, alpha = 85)
                drawGoldHexFrame(canvas, width, height, inset = 76f, alpha = 110)
            }
            "babyshower_04" -> {
                fillGradient(canvas, width, height, Color.parseColor("#EDE7F6"), Color.parseColor("#FAF7FF"))
                drawGarland(canvas, width, 108f)
                drawSoftClouds(canvas, width, height - 150f)
                drawStarAccent(canvas, width / 2f, 100f)
                drawGoldHexFrame(canvas, width, height, inset = 70f, alpha = 125)
            }
            else -> {
                fillGradient(canvas, width, height, Color.parseColor("#EFEBE9"), Color.parseColor("#FAF8F6"))
                drawBohoAccent(canvas, width, height)
                drawGoldHexFrame(canvas, width, height, inset = 80f, alpha = 95)
            }
        }
    }
    ...
    private fun drawGoldOvalFrame(canvas: Canvas, cx: Float, cy: Float, rx: Float, ry: Float) {
        ...
        val inner = Paint(stroke).apply {
            strokeWidth = 2f
            color = Color.argb(160, 255, 215, 0)
        }
        canvas.drawOval(RectF(cx - rx + 12f, cy - ry + 10f, cx + rx - 12f, cy + ry - 10f), inner)
    }

    private fun drawLanternString(canvas: Canvas, width: Int, top: Float, warm: Boolean = false) {
        val line = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = Color.argb(120, 80, 60, 40)
            strokeWidth = 2f
            style = Paint.Style.STROKE
        }
        canvas.drawLine(40f, top, width - 40f, top + 8f, line)
        val count = 5
        for (i in 0 until count) {
            val x = 80f + (width - 160f) * i / (count - 1).coerceAtLeast(1)
            val y = top + 18f + (if (warm) 6f else 0f)
            drawDiya(canvas, x, y)
        }
    }

    private fun drawSoftClouds(canvas: Canvas, width: Int, top: Float) {
        val cloud = Paint(Paint.ANTI_ALIAS_FLAG).apply { color = Color.argb(170, 255, 255, 255) }
        listOf(70f, width / 2f - 60f, width - 130f).forEach { cx ->
            canvas.drawOval(RectF(cx, top, cx + 90f, top + 36f), cloud)
            canvas.drawOval(RectF(cx + 28f, top - 10f, cx + 78f, top + 28f), cloud)
        }
    }

    private fun drawStarAccent(canvas: Canvas, cx: Float, cy: Float) {
        val paint = Paint(Paint.ANTI_ALIAS_FLAG).apply { color = Color.parseColor("#FFD54F") }
        val path = Path().apply {
            moveTo(cx, cy - 16f)
            lineTo(cx + 5f, cy - 4f)
            lineTo(cx + 16f, cy - 4f)
    ...
    private fun drawWreathRing(canvas: Canvas, cx: Float, cy: Float, radius: Float) {
        val leaf = Paint(Paint.ANTI_ALIAS_FLAG).apply { color = Color.parseColor("#66BB6A") }
        for (i in 0 until 16) {
            val angle = i * (Math.PI * 2 / 16)
            val x = cx + cos(angle).toFloat() * radius
            val y = cy + sin(angle).toFloat() * radius * 0.75f
            canvas.drawOval(RectF(x - 10f, y - 6f, x + 10f, y + 6f), leaf)
        }
    }

    private fun drawBohoAccent(canvas: Canvas, width: Int, height: Int) {
        val grass = Paint(Paint.ANTI_ALIAS_FLAG).apply { color = Color.parseColor("#BCAAA4") }
        var x = 50f
        while (x < width - 40f) {
            val path = Path().apply {
                moveTo(x, height - 90f)
                quadTo(x + 8f, height - 150f, x + 4f, height - 200f)
            }
            val stroke = Paint(grass).apply {
                style = Paint.Style.STROKE
                strokeWidth = 3f
            }
        ...
        canvas.drawArc(RectF(width / 2f - 70f, 120f, width / 2f + 70f, 200f), 200f, 140f, false, arc)
    }
}
